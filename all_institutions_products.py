...
# Reconciliation: H032+H033+H034+H035 should equal RIAD4080 (TTM basis)
# for banks that report all four Memo-15 components.
rec = combined.loc[is_bank,
                   fee_ttm_cols + ["total_fees_ttm_4q"]].dropna()
if len(rec):
    diff = (rec[fee_ttm_cols].sum(axis=1)
            - rec["total_fees_ttm_4q"]).abs()
    ok = (diff <= 2000).sum()   # within $2k allows for rounding in source
    ...
    if ok < len(rec):
        ...
        print("  Largest reconciliation gaps (abs $):")
