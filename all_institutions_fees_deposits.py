...
# Point-in-time deposits in dollars (RCON2200 is in thousands)
...
for _, ncua_date, label in PERIODS:
    ...
    if label in TTM_LABELS:
        codes += ["ACCT_131"]
    if label == CURRENT:
        codes += ["ACCT_018"]     # total shares and deposits
...
# Point-in-time deposits (already in dollars)
...
# =====================================================================
# 3. Combine; deposit share vs COMBINED bank + CU denominator
# =====================================================================
combined = pd.concat([banks, cus], ignore_index=True)

# One denominator for all institutions: total bank domestic deposits
# (RCON2200, in dollars) + total credit union shares/deposits (ACCT_018).
# Computed on the dollar-converted column so units are consistent.
total_deposits_all = combined["total_deposits_20260630"].sum()
combined["pct_of_total_deposits"] = (
    combined["total_deposits_20260630"] / total_deposits_all * 100
)
...
out_path = os.path.join(DATA_DIR, "all_institutions_fees_deposits.csv")
...
print(f"  Combined deposit share column sums to "
      f"{combined['pct_of_total_deposits'].sum():.1f} (should be ~100)")
print(f"  Bank share of combined deposits: "
      f"{combined.loc[is_bank, 'pct_of_total_deposits'].sum():.1f}%")
print(f"  CU share of combined deposits: "
      f"{combined.loc[~is_bank, 'pct_of_total_deposits'].sum():.1f}%")
