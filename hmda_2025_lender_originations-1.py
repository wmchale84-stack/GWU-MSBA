...
if panel_matches:
    ...
    # Respondent-level RSSD preferred over parent/topholder RSSDs
    rssd_cols = [c for c in panel.columns if "rssd" in str(c).lower()]
    rssd_col = next((c for c in rssd_cols
                     if "parent" not in str(c).lower()
                     and "topholder" not in str(c).lower()),
                    rssd_cols[0] if rssd_cols else None)
    tax_col = next((c for c in panel.columns
                    if "tax" in str(c).lower()), None)
    if lei_col and name_col:
        take = [lei_col, name_col]
        rename = {lei_col: "lei", name_col: "lender_name"}
        if rssd_col:
            take.append(rssd_col)
            rename[rssd_col] = "rssd"
        if tax_col:
            take.append(tax_col)
            rename[tax_col] = "tax_id"
        names = (panel[take]
                 .drop_duplicates(subset=lei_col)
                 .rename(columns=rename))
        out = out.merge(names, on="lei", how="left")
        id_cols = [c for c in ("lei", "lender_name", "rssd", "tax_id")
                   if c in out.columns]
        out = out[id_cols + [c for c in out.columns
                             if c not in id_cols]]
        print(f"Lender names merged from: {os.path.basename(ppath)}")
        print(f"  RSSD column: {rssd_col}; tax ID column: {tax_col}")
    else:
